...
// https://github.com/KilledByAPixel/ZzFX (MIT licensed)
//
// Vendored NEAR-VERBATIM (only `let` -> `export let` changed) from the
// project's official ZzFXMicro.min.js, per js13kgames.com/resources'
// "Sound and music" list - a tiny (~600b before further terser/roadroller
// squeeze), extensively used/tested js13k sound engine designed to be
// pasted into a project as-is rather than hand-reimplemented. The
// single-letter params and the `k=[]` array/number type-punning trick are
// intentional code-golf from upstream, not a bug - hence @ts-nocheck for
// this file specifically instead of trying to make it strict-mode clean.
// See dis_doc.md's "Звук" section for why sound was scoped to a few short
